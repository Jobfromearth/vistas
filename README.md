...
- **Fully open dataset.** The versioned snapshot ships under an open license (CC0/CC-BY) — copy it, fork it, build on it. See [ADR-0004](docs/adr/0004-fully-open-dataset.md).

## Status

Implemented and tested end-to-end against the real [Riksdagen open data API](https://data.riksdagen.se):

| Milestone | What | Status |
|---|---|---|
| M1 — data pipeline | Riksdagen SFS ingestion (Utlänningslagen 2005:716), §-level chunking with legal-date extraction, bitemporal version chains | ✅ done |
| M2 — MCP minimal | FTS5 lexical retrieval, stdio server | ✅ done |
| M3 — versioning tools | `rule_timeline`, `recent_changes` (ahead of schedule) | ✅ done |
...
| M6 — expansion | Cross-border tax (Skatteverket/SINK), Migrationsverket guidance sources | ⏳ not implemented |

**Compliance status:** unlike statute text, agency-authored prose isn't automatically copyright-free, so per [ADR-0004](docs/adr/0004-fully-open-dataset.md) full-text redistribution required verifying each site's terms first. That verification is now done for migrationsverket.se's main site — full-text redistribution under **CC-BY** (attribution required) is confirmed permitted, see [`docs/research/migrationsverket-villkor.md`](docs/research/migrationsverket-villkor.md). Scraping/ingesting that content is still to-do (pure engineering now, not a compliance blocker). The `rättsliga ställningstaganden` legal-position documents (hosted on the separate Lifos platform) remain unverified and stay under the conservative default (abstract + link only) until checked separately.
...
cd vistas
uv sync
uv run vistas-build          # builds a local snapshot from Riksdagen
